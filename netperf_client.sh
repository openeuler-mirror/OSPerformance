...
if ls ../../report/netperf
then
    echo "netperf_results已经存在,即将帮你删除。"
    sleep 3
    rm  -r ../../report/netperf
else
    echo "netperf test results不存在，拷贝netperf测试结果。"
    sleep 2
fi

mkdir -p ../../report/netperf/
cp TCP_STREAM.txt ../../report/netperf/
cp UDP_STREAM.txt ../../report/netperf/
cp TCP_RR.txt ../../report/netperf/
cp TCP_CRR.txt ../../report/netperf/
cp UDP_RR.txt  ../../report/netperf/
cp omni.txt ../../report/netperf/
